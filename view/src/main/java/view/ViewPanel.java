...
class ViewPanel extends JPanel implements Observer {
	...
	protected void paintComponent(final Graphics graphics) {
		...
		for(int y = ((this.getHeight()/2)-(map.getHeight()/2)); y<((this.getHeight()/2)+(map.getHeight()/2)); y++){
			for(int x = ((this.getWidth()/2)-(map.getLenght()/2));x<((this.getWidth()/2)+(map.getLenght()/2));x++){
